...
[Andy Matuschak](https://andymatuschak.org/)[![Twitter logo](https://andymatuschak.org/prompts/twitter_logo.svg)](https://twitter.com/andy_matuschak)
...
- [Open lists](https://andymatuschak.org/prompts/#open-lists)
- [Salience prompts](https://andymatuschak.org/prompts/#prompting-salience)
- Prompt-writing, in practice
  - [Iterative prompt-writing](https://andymatuschak.org/prompts/#iterative-prompts)
  - [Litmus tests](https://andymatuschak.org/prompts/#litmus)
  - [Revising prompts over time](https://andymatuschak.org/prompts/#revising)
...
这种幻想可能来自于玩太多电子游戏。但我在使用间隔重复系统时的感觉惊人地相似。在最佳状态下，这些系统感觉就像是魔术。[^1]记忆不再是一种随意的现象，这是你希望发生的事情：间隔重复系统让记忆成为一种选择。如果使用得当，它们可以加速学习，促进创造性工作，[和more](https://notes.andymatuschak.org/Spaced_repetition_systems_can_be_used_to_program_attention).。但就像我童年时的白日梦一样，只有当你按下正确的键序列，产生正确的咒语时，这些奇迹才会显现。也就是说，当你设法写出好的提示时--你在练习会上复习的问题和答案。
...
间隔重复学习任务最常见的改变机制被称为*提取练习。*简而言之：当你试图从记忆中回忆一些知识时，提取的行为往往会强化这些记忆。更多背景信息，参见罗迪格和卡皮克，[The Power of Testing Memory](https://andymatuschak.org/prompts/Roediger2006.pdf)(2006年)]。Gwern Branwen[关于Spaced repetition](https://www.gwern.net/Spaced-repetition)的文章是一个很好的流行概述。你会更慢地忘记这些知识。通过在一段时间内有策略地间隔几次检索，您可以有效地阻止遗忘。物理机制尚不清楚，但数百名认知科学家已经通过实验探索了这一效应，他们在不同的学科、知识类型(事实、概念、程序、运动)和测试方式(选择题、简答题、口试)上重现了中心发现。

The value of fluent recall isn’t just in memorizing facts. Many of these experiments tested students not with parroted memory questions but by asking them to make inferences, draw concept maps[^3], or answer open-ended questions. In these studies, improved recall translated into improved general understanding and problem-solving ability.
...
这些嵌入的提示是正在进行的研究项目的一部分。在助记介质中的第一个实验是*[量子国](https://quantum.country/)*，)，这是量子计算的入门读物。量子国家是具体的和技术的：定义、符号、定律。相比之下，本指南主要介绍启发式、心智模型和建议。因此，嵌入式提示在这两个上下文中扮演着截然不同的角色。你可能不需要在这里记住准确的定义，但我相信这些提示会帮助你内化指南的想法并将其付诸实践。另一方面，本指南的材料可能过于偶然和过于个人化，无法从作者提供的提示中获益。这是一个实验，我邀请你[告诉我你的经历](邮件地址：Andy@andymatuschak.org)。
...
一个重要的限制是值得注意的。本指南描述了如何编写提示语来产生和加强您自己的理解，超出了作者明确提供的范围。Orbit还没有为读者提供混合作者提供的提示或添加他们自己的提示的能力。未来的工作将朝着这个方向扩展系统。
...
[^3]: See e.g. Karpicke and Blunt, [Retrieval Practice Produces More Learning than Elaborative Studying with Concept Mapping](https://andymatuschak.org/prompts/Karpicke2011.pdf) (2011); and Blunt and Karpicke, [Learning With Retrieval-Based Concept Mapping](https://andymatuschak.org/prompts/Blunt2014.pdf) (2014).
[^4]: For more background on the mnemonic medium, see Matuschak and Nielsen, [How can we develop transformative tools for thought?](https://numinous.productions/ttft/) (2019).
[^5]: If you have an existing spaced repetition practice, you may find it annoying to review prompts in two places. As Orbit matures, we’ll release import / export tools to solve this problem.
[^6]: This effect has been produced in many experiments but is not yet well understood. For an overview, see Murayama et al, [Forgetting as a consequence of retrieval: a meta-analytic review of retrieval-induced forgetting](https://andymatuschak.org/prompts/Murayama2014.pdf) (2014).
[^7]: SuperMemo’s algorithms (also used by most other major systems) are tuned for 90% accuracy. Each review would likely have a larger impact on your memory if you targeted much lower accuracy numbers—see e.g. Carpenter et al, [Using Spacing to Enhance Diverse Forms of Learning](https://andymatuschak.org/prompts/Carpenter2012.pdf) (2012). Higher accuracy targets trade efficiency for reliability.
[^8]: For more on the notion that difficult retrievals have a greater impact than easier retrievals, see the discussion in Bjork and Bjork, [A New Theory of Disuse and an Old Theory of Stimulus Fluctuation](https://andymatuschak.org/prompts/Bjork1992.pdf) (1992). Pyc and Rawson, [Testing the retrieval effort hypothesis: Does greater difficulty correctly recalling information lead to higher levels of memory?](https://andymatuschak.org/prompts/Pyc2009.pdf) (2009) offers some focused experimental tests of this theory, which they coin the “retrieval effort hypothesis.”
[^9]: If you’re a vegetarian, I hope you can look past the discussion of bones: choosing this example involved many trade-offs.
[^10]: As [Michael Nielsen notes](http://augmentingcognition.com/ltm.html), similar lightweight prompt-writing goals can enliven seminars, professional conversations, events, and so on.
[^11]: For a vivid account of this process in mathematics, see Michael Nielsen, [Using spaced repetition systems to see through a piece of mathematics](http://cognitivemedium.com/srs-mathematics) (2019).
[^12]: Piotr Wozniak, a pioneer of spaced repetition, has been developing a system he calls [incremental reading](https://supermemo.guru/wiki/Incremental_reading) which attempts to actively support this kind of iterative, incremental prompt writing.