...
- The central role of retrieval practice 提取练习的核心角色
  - Properties of effective retrieval practice prompts 有效提取练习提示的性质
...
- Open lists 开放式列表
- Salience prompts 显著性提示
- Prompt-writing, in practice 提示写作，在实践中
  - Iterative prompt-writing 迭代提示写作
  - Litmus tests 试金石测试
  - Revising prompts over time 随着时间改写提示
...
间隔重复学习任务最常见的改变机制被称为**提取练习**。简而言之：当你试图从记忆中回忆一些知识时，提取的行为往往会强化这些记忆。[^3] 你会更慢地忘记这些知识。通过在一段时间内有策略地间隔几次提取，您可以有效地阻止遗忘。物理机制尚不清楚，但数百名认知科学家已经通过实验探索了这一效应，他们在不同的学科、知识类型(事实、概念、程序、运动)和测试方式(选择题、简答题、口试)上重现了中心发现。
...
流畅回忆的价值不仅仅在于记忆事实。许多实验不是用鹦鹉式的记忆问题测试学生，而是让他们进行推理，画概念图[^4]，或者回答开放式问题。在这些研究中，记忆力的提高转化为总体理解和解决问题能力的提高。
...
**提取**是这种有效的练习方式与典型的学习习惯相区别的关键因素。简单地「提醒」自己一些材料(比如重读)会让你的记忆力和解决问题的能力变得更弱。提取产生的学习被称为「测试效应」，因为它发生在你显性地测试自己的时候，从内心去回忆你头脑中纠结的一些知识。这类考试看起来像典型的学校考试，但从某种意义上说，它们恰恰相反：提取练习是为了测试你的知识来**产生**学习，而不是评估学习。
...
间隔重复系统就是为了促进这一效果而设计的。如果你想要提示语来加强你对某个主题的理解，你必须学会写提示语，这些提示语可以共同调用所有关键细节的提取练习。
...
本指南是 Michael Nielsen 和我所称的**助记媒介**[^5]的一个例子。它通过直接插入到文本中的间隔重复提示来举例说明自己的建议。如果你正在阅读这篇文章，你可能已经使用过间隔重复系统。本指南的系统 [Orbit](https://withorbit.com/) 的工作原理与此类似[^6]。但它还有一个更深层次的愿望：通过将专家撰写的提示整合到阅读体验中，作者可以写出读者可以相对较少的努力就能深刻内化的文本。如果您是一名作者，那么本指南可能会帮助您学习如何为您的个人实践和您使用 [Orbit](https://withorbit.com/)。撰写的出版物编写良好的提示。当然，您可以阅读本指南，而不需要回答嵌入的提示，但我希望您能试一试。
...
这些嵌入的提示是正在进行的研究项目的一部分。在助记介质中的第一个实验是《[量子国度](https://quantum.country/)》，这是量子计算的入门读物。量子国度是具体的和技术的：定义、符号、定律。相比之下，本指南主要介绍启发式、心智模型和建议。因此，嵌入式提示在这两个上下文中扮演着截然不同的角色。你可能不需要在这里记住准确的定义，但我相信这些提示会帮助你内化指南的想法并将其付诸实践。另一方面，本指南的材料可能过于偶然和过于个人化，无法从作者提供的提示中获益。这是一个实验，我邀请你[告诉我你的经历](邮件地址：Andy@andymatuschak.org)。
...
### Properties of effective retrieval practice prompts 有效提取练习提示的属性
...
当编写间隔重复提示以调用提取练习时，您所做的操作类似于语言翻译。你在问：哪些任务在总体执行时，需要点亮灯泡，而这些灯泡是当你把这个想法「完全装满」脑海中时被激活的？
...
提取练习机制隐含着有效提示的一些核心属性。我们将在这里简要回顾它们，本指南的其余部分将通过许多示例来说明它们。
...
提取练习提示应该**有重点。**涉及太多细节的问题或答案会影响你的注意力，刺激不完整的提取，导致一些灯泡没有亮起。没有重点的问题也会让你更难检查你是否记住了答案的所有部分，也更难记下你有什么不同之处。通常情况下，最好一次只关注一个细节。
...
提取练习提示应该**精确**说明他们所要求的内容。模糊的问题会引出模糊的答案，这不会可靠地点亮你的目标灯泡。
...
提取练习提示应该产生**一致**的答案，每次执行任务时都会亮起相同的灯泡。否则，你可能会遇到一种被称为「提取诱导遗忘」[^7]的干扰现象：你在练习中记住的东西会得到加强，但你没有记住的其他相关知识实际上是被抑制的。现在，有一种有用的提示类型，每次重复都会产生新的答案，但这种提示利用了不同的变化理论。我们稍后将在本指南中简要讨论它们。
...
提取练习提示应该**易于处理**。为了避免在复习过程中因干扰而造成的混乱和反复出现的烦恼，你应该努力写出你几乎总是能正确回答的提示语。这通常意味着将任务分解，或者添加线索。[^8]
...
提取练习提示应该**费力**。提示实际上涉及到从记忆中提取答案，这一点很重要。你不应该能够简单地推断出答案。提示是有帮助的，我们将在后面讨论——只是不要「泄露答案」。事实上，努力似乎是影响提取练习效果的一个重要因素。[^9] 随着时间的推移，这是间隔审查的一个动机：如果回忆答案太容易，提取练习几乎没有什么效果。
...
要实现这些特性，主要是要编写范围严格的问题。当提示符的范围太广时，您通常会遇到问题：提取通常会缺少重点目标；您可能会产生不准确或不一致的答案；您可能会发现提示符难以处理。但是，写范围狭窄的问题出人意料地困难。你需要把知识分解成离散的部分，这样你就可以把这些部分重新构建起来，作为提取练习的提示。这种分解也使审查更有效率。课程表将快速删除常规练习中容易的材料，同时确保您经常检查您认为最难的组件。
...
因此，要写出有效的提取练习提示，我们必须学习两项技能：如何准确地描述我们要强化的知识，以及如何提出问题来强化这些知识。
...
为了更具法式风味，可以用韭菜代替芹菜，加入月桂叶、黑胡椒和百里香。为了更深层次的味道，先把骨头和蔬菜烤成所谓的「棕色鸡汤」（上面的菜谱是用来做「白鸡汤」的，它更细腻，但也更多样）。
...
在本指南中，我们会设想你是一个有兴趣的家庭厨师，以前从未做过高汤。自然，如果你是一个有经验的厨师，你可能只需要这些提示中的几个。当然，如果你根本不做饭，你也就不会写这些提示了 试着把这些例子看成是对你如何在事先没有太多流畅性的情况下深入内化一项资源的演示[^10] 。
...
## How to make and store chicken stock: factual and procedural knowledge 如何制作和储存鸡汤：事实性和程序性的知识
...
我们将从这段话的「菜谱」部分开始，它描述了如何制作和储存鸡汤。将这一部分的文字与更多的概念性的初始段落进行对比是很有意思的。作为一种形式，菜谱已经涉及到比你在普通散文中发现的更明确的知识结构。这将给我们提供一个开始的脚手架。
...
写这样一个简单的事实性提示，自然会勾起你可能考虑添加的邻居：*解释*提示。当一个细节似乎很有挑战性，或者解释本身很有趣时，我就写这样的提示。一个更有经验的厨师可能不会理会第一个问题，但他们可能仍然会发现这个问题很有用：
...
解释问题将强化事实问题所捕获的知识。也许更重要的是，解释使事实更有意义。像这样的提示提供了一个钩子，将事实与你在以后的烹饪冒险中可能获得的其他想法联系起来。例如，一道中国的果冻鸡脚可能会启发你尝试用鸡脚作为汤汁中的一些骨头（因为事实上，它们是最富含明胶的部分）。
...
请注意，答案是在文本的不同部分找到的! 写作提示往往涉及到跳来跳去，因为你要努力找出拼图的碎片并把它们放在一起。
...
现在，这个解释提示是一个很好的初步尝试，但它可以更精确。用「因为骨头很经济」来回答「我们为什么用骨头做鸡汤？」是合理的。你想写的问题要能使你毫不含糊地提取到你心中的信息。这种更精确的提示比较好：
...
你可能会注意到，这组评论中的一个提示没有满足一致性属性。那条提示并不是要参与提取实践，至少不像我们目前看到的那样。也许它的效果不是很好，这样的提示有点像一个实验! 我们稍后会进一步讨论它们。
...
回到配料表上。鸡汤显然涉及鸡肉和水：这来自于汤的定义（我们将在后面处理）。但是，还有所有的芳香调味品，这些调味品的变化较大，在一定程度上取决于应用。从功能分组的角度来理解这些成分，将有助于我们内化配方的结构：

> Q. Chicken stock is made with chicken, water, and what other category of ingredients?
>
> A. Aromatics.

> 问：鸡汤是用鸡肉、水和其他什么类别的成分制成的？
>
> 答：芳香剂。
...
但是，除非你是一个有经验的厨师，否则你可能会发现这个提示是难以解决的，或者你可能记得成分不一致。像这样的无序列表，要转化为好的提示语是很有挑战性的。
...
...以此类推。请注意，我保持列表的顺序不变。当每个元素都有一个一致的（尽管是任意的）位置时，当你重复这些提示时，你最终会在某种程度上学会列表的视觉「形状」。这将有助于你的记忆。
...
大多数间隔重复软件都有特殊功能，可以快速生成类似这样的一组填空提示。在软件的界面中，这些提示通常被称为「填空」（cloze deletions）。在每个复习环节中，软件只会要求你填一个空。这种行为很重要，因为如果没有它，一个变体就会「泄露」给另一个变体的答案。
...
当然，归根结底，你希望能够按要求回忆起所有的成分，而不是只有一种。很多时候，这些单元素的删除就足以实现这一目标。但对于复杂的想法，你可能会发现你需要在彻底练习了它们的独立成分后再增加综合提示。在列表的情况下，你可以想象，随着你对单个条目记忆的提高，系统可以要求你同时填入更多的空白处。我不知道有什么一般的记忆系统能做到这一点，但这似乎值得尝试。这是一个将更复杂的知识建模纳入这些系统的一般机会的例子。
...
另一个帮助自己理解这样的列表的方法是为每个组成部分写出解释提示。快速回答：胡萝卜提供植物性的甜味；像盐一样，这种糖可以提亮其他味道。*为什么*胡萝卜是鸡汤的好香料？如果你知道每种成分的答案，你就会更容易按要求生成清单，也许就不会有任何删减。和简单的事实一样，解释使知识更有意义。在这种情况下，菜谱上没有说，所以你需要做一些研究来写这种问题。

### Cues and elaborative encoding 提示和精心设计的编码

If you find yourself struggling with these prompts, it can be helpful to add a cue, like this:

如果你发现自己在这些提示中挣扎，添加一个提示是很有帮助的，就像这样：
...
但要确保这个提示不会使提示变得微不足道：重要的是，你要花点力气从记忆中提取出答案。考虑一下这个提示：
...
这个提示不要求你提取任何有关鸡汤的知识：只有一种蔬菜与「鹦鹉」押韵。你可以不看菜谱就回答这个问题。相比之下，前一个提示的线索——「草药」——让你的记忆有了更多的工作要做。有许多草药，你仍然需要记住菜谱上指定的哪一种。
...
也就是说，「与鹦鹉押韵」可能是一个有用的记忆工具，可以添加到*答案中，提供一个联想，以帮助你在未来：
...
这类提示涉及认知科学家们通过实验探索的另一种记忆现象：当你把信息与其他记忆联系起来时，你会使信息更容易回忆起来。这个过程被称为*协作性编码*[^11]。一份配料表的成员可能很难与任何有意义的东西联系起来。在这种情况下，你仍然可以通过编造一个联想作为记忆工具来利用精心设计的编码。生动的联想效果最好，所以找到涉及视觉、有意义的个人经历或幽默和厌恶等情绪的关系很有帮助。
...
当你想记住的信息看起来相对随意或孤立时，把这些记忆方法放在答案栏里是一个有用的一般技巧。通过把联想放在括号里，你可以清楚地看到提示的重点是记住答案：联想是一个额外的花絮，你可以参与，也可以不参与。
...
这个特殊的提示不太可能需要任何额外的帮助，但是如果你真的发现自己在挣扎，你可以增加一个专门用于加强联想的提示：
...
你也可以通过在提示语中加入图像来利用精心设计的编码，而不是制造一种记忆装置。同样的填空想法也很适用于图像。你可能会发现这个提示比它的文字内容更容易记住。
...
当然，提示和精心设计的编码会增加额外的努力。你不需要一直使用它们：你可能会觉得很累。但是，如果你怀疑一个提示难以记住，或者你注意到自己在实践中的挣扎，那么它们就是有用的技术。
...
好吧，你可以尝试这样的方法。「一批鸡汤里有多少鸡骨？」 但什么是「一批」？这个问题还不够精确。我们可以求助于特定的配方，问 「安迪的鸡汤配方里有多少鸡骨？」 但这实际上不是我们想知道的：菜谱指出，你要根据你使用的鸡骨数量调整菜谱。写出好的提示往往涉及解释，这是创造超越明确书写的理解的第一步。
...
那么欧芹呢？这里是另一个解释的例子。无论如何，「一束」欧芹不是一个一致的单位。所以我不会费心写关于欧芹数量的提示，我只抓一把。
...
请注意我在这里把成分表分成了许多问题，每个问题都很集中，很精确。我注意到，人们常常感到不得不节约他们所写的提示的数量。提示语似乎有一个单位的「价格」，所以人们自然而然地试图写更少的问题，覆盖更多的领域。但这是适得其反的。除非你明确决定排除某些信息，否则「原始知识单位」的数量是固定的，是地域的常数。当你用较少的数量写出较粗的提示时，你并没有减少你必须学习的数量。你只是让材料更难复习。
...
提示语比你可能想象的要便宜。一个简单的提示在整个第一年的练习中会消耗10-30秒的时间，而在随后的每一年则会少很多。在你内化这一观察结果之前，试着采用这一经验法则：写更多的提示语，而不是感觉自然。
...
现在，提示语很便宜，但它们不是*免费的。*除了时间成本，它们还有情感成本：没有人愿意花时间复习一堆他们已经知道的无聊材料。因此，如果你为一个已经相当熟悉的主题写提示语，你应该使用较少的提示语——不是因为为熟悉的主题写较粗的问题总是安全的，而是因为你需要捕捉的边缘知识较少。
...
例如，你可能会注意到，我没有写关于在向汤汁中添加原料之前如何准备的提示语。没有必要，我觉得：在使用这些成分作为芳香剂之前，粗略地切碎蔬菜和捣碎大蒜是大多数认真做汤的厨师的自然推断。但是，如果这对你来说不是一个微不足道的推论，那就提示吧。
...
与此相关，「重点」提示的适当规模取决于你已经内化的概念的规模。这组特殊的芳香剂是如此深入人心，以至于我在写提示时将其作为一个单位（「意大利芳香剂」），而不是记住单个成分和数量。
...
随着你对越来越复杂的概念的熟练掌握，你可以写出越来越复杂的提示，同时使每个提示都集中在感觉上是一个单一的细节。事实上，用越来越复杂的「块」来思考的能力似乎是专业知识的重要组成部分。[^12] 从这个角度来看，记忆系统的一个作用是加速增加你在一个主题中的有效块的大小。
...
现在我们已经有了关于成分的提示，让我们来看看菜谱的步骤。这是*程序性*的知识——执行具体任务所需的知识，更多的是知道如何做而不是知道做什么。
...
从某种意义上说，程序就是清单。因此，我们可以从使用我们用于配料表的cloze-deletion方法开始：
...
这是一个非常不集中的提示。它包括许多不重要的细节，这些细节分散了你真正打算提取的知识。根据我的经验，像这样冗长的提示往往会使我注意力不集中，产生模糊不清、心不在焉的答案。
...
我们可以通过简单地删除单词来在一定程度上改进这个提示。如果有必要，我们可以增加额外的提示，以涵盖我们删除的任何细节。
...
这样做更好。但是将程序编辑下来有助于我们对这些知识进行一些观察。首先：几个关键词（或词组）承载了该程序的关键细节。**缓慢地**到**小火，然后保持**小火**90分钟**如果你知道这四个粗体字，你基本上就知道这个程序了。其他的话只是一个骨架。这给我们带来了第二个看法：第一和第四步不值得写提示。如果你知道什么是高汤（我们会讲到这一点），你就知道你是在锅里熬制一堆材料。当汤汁炖完后，最后一步就是常识了。你还会做什么？
...
以这种方式突出关键词后，写提示语往往感觉像在玩危险游戏：你能以问题的形式来表述吗？下面是一组修订后的问题，它反映了我对这一程序的理解。
...
在我们的高汤配方中，动词并不十分重要：「拿来」、「放下」、「过滤」。你在不同温度的水中烹饪食材，所以这些动作是显而易见的。但是，描述何时在动词之间移动的条件或启发式方法是很重要的：首先是当水达到小火时，然后是90分钟过去后。虽然不值得捕捉你正在加热锅的事实，但副词「缓慢」确实很重要。
...
这个配方是相当线性的，但更复杂的程序可能会有分支，包括可能触发流程图中的一些替代路径的条件或特殊情况。这样的谓词结构通常是值得捕捉的。如果分支足够复杂，你可以考虑画一个流程图并在提示中使用。
...
在学习程序时，解释型的提示特别有价值：它们可以帮助你避免死记硬背，建立更深刻的理解。请注意，在这个特定的案例中，我们只能够穿越「为什么」的一个层次。为什么要小火？菜谱上说，是为了使味道更干净。为什么小火能产生更干净的味道？它没有说。你可能会发现自己想知道「更亮」或「更干净」到底是什么意思。写下这个提示后，我们发现了一个理解上的差距：多么有用啊！我们可以深入研究这个问题，但是，我们不知道该怎么做。我们可以挖掘这个问题，但我们会选择暂时继续前进。我们通过明确地将答案表述为暂定的，依赖于外部的主张来表明这一点。
...
每当你在处理主观的、临时的或不完整的信息时，在你的提示中加入类似的警告语是很有帮助的。从某种意义上说，我们所写的大部分提示都是临时性的：其他菜谱可能会有一些不同的做法。因此，我们可以把一切都表述得更暂定，但在实践中，这比它的价值更让人分心。通常，记录信息的来源就足够了。大多数间隔重复系统都有元数据字段，你可以用来将这些提示与原始鸡汤菜谱联系起来。

### Exercise: how to store chicken stock? 练习：如何储存鸡汤？
...
我们已经写了关于如何制作鸡汤的提示。现在轮到你了。利用我们到目前为止所学到的知识，在储存笔记上写出代表你对这一段的理解的提示语：
...
这将产生大约1.5qt.鸡汤在冰箱里可以保存一个星期，或在冰箱里无限期地保存。冷藏后，鸡汤上会有一层脂肪，在使用鸡汤前请将其撇去，并在任何美味的烹饪情况下用脂肪代替油或黄油。
...
**属性和倾向**：是什么让高汤成为高汤？对于高汤来说，什么是永远的、有时的、和永远的？
...
假设你正在阅读一篇看起来很有趣的文章。试着给自己设定一个易懂的目标：在你第一次阅读时，要写少量的提示语（比如5-10条），内容是看起来最重要的、有意义的或有用的。
...
我发现，这样的目标会改变我的阅读方式，即使是随意的文本。当我第一次采用间隔重复练习时，我觉得我「应该」写下所有的提示语。这使得阅读成为一件苦差事。相比之下，每次只写几个关键的提示，感觉很自由。[^17]我读得更积极了，注意到脑海中有一种痒痒的感觉。"哦，那是一个多汁的部分！让我们抓住那个！"。让我们抓住这一点！"
...
*不鼓励模式匹配。*如果你写了一个长的问题，其中有不寻常的词或提示，你最终可能会记住这个问题的*形状，并学会其相应的答案——不是因为你真的在思考相关的知识，而是通过机械的模式联想。Cloze删除似乎特别容易受到这个问题的影响，尤其是在通过复制和编辑文本中的段落来创建。避免这种情况的最好办法是保持问题的短小和简单。
...
**错误的否定：**你怎么可能知道提示想要捕捉的信息，但却**没有产生正确的答案？这种失败往往是由于没有包括足够的背景。
...
当你在写提示语时，往往很难诊断出问题。问题可能只有在审查时才会显现出来，有时只有在一个提示的重复间隔时间长达数月时才会显现。提示语写作涉及漫长的反馈循环。因此，正如随着时间的推移逐步编写提示语一样，随着时间的推移逐步修改提示语也很重要，因为你会发现问题和机会。
...
在你的审查会议上，要警惕对一个提示的内部「叹息」的感觉。通常你会想："哦，天哪，这个提示——我永远都记不住答案。「或者」每当这个提示出现时，我都知道答案，但我并不真正理解它的含义"。倾听这些反应并利用它们来推动你的复习。为了避免干扰你的复习课程，大多数间隔重复系统都允许你在复习过程中标记一个需要修改的提示。然后，一旦你的会议结束，你可以查看被标记的提示列表，并改进它们。
...
我认为，在间隔重复练习中，最重要的是要「优化」你的复习课及其内容的情感联系。值得学习的是如何创建有效代表多种不同理解的提示，但一个提示并不因为它满足了我在这里描述的所有属性就值得复习。如果你发现自己在审查一些你不再关心的东西，你应该采取行动。有时经过反思，你会想起为什么你一开始就关心这个想法，你可以修改提示，以提示这个动机。但大多数时候，修改这种提示的正确方法是删除它们。
...
- Michael Nielsen的[增强长期记忆](http://augmentingcognition.com/ltm.html)：彻底回顾了间隔重复的作用，以及为什么你应该关心这个问题；更详细地介绍了如何实际地将提示写作融入你的阅读实践，特别是在阅读学术文献时；关于创造性应用的说明；以及更多。
- 迈克尔-尼尔森的[利用间隔重复系统看透数学作品](http://cognitivemedium.com/srs-mathematics)展示了如何利用间隔重复的提示作为杠杆，反复加深对数学作品的理解。
...
- Soren Bjornstad的[记忆系统系列](https://controlaltbackspace.org/memory/designing-precise-cards/)涵盖了维护提示库的许多实用主题，包括一些关于提示书写的建议。
- Nicky Case的[How To Remember Anything Forever-ish](https://ncase.me/remember/)介绍了间隔重复法，并通过有趣的插图介绍了一些提示写作的技巧。
- 我们怎样才能开发变革性的思想工具？](https://numinous.productions/ttft/)（来自Michael Nielsen和我）讨论了提示写作的挑战，特别关注「记忆媒介」，其中包括将提示嵌入叙事性散文中。
...
感谢Peter Hartree、Michael Nielsen、Ben Reinhardt和Can Sar对本指南的有益反馈；感谢我在编写本指南时举办的提示性写作研讨会的许多与会者；感谢Gwern Branwen和Taylor Rogalski对提示性写作的有益讨论，这些讨论为本工作提供了信息。我特别感谢迈克尔-尼尔森（Michael Nielsen）多年来围绕记忆系统进行的对话和合作，这些对话和合作塑造了我对这一主题思考的各个方面。
