...
提取练习提示应该**费力**。提示实际上涉及到从记忆中提取答案，这一点很重要。你不应该能够简单地推断出答案。提示是有帮助的，我们将在后面讨论——只是不要「泄露答案」。事实上，努力似乎是影响提取练习效果的一个重要因素。[^9] 随着时间的推移，这是间隔复习的一个动机：如果回忆答案太容易，提取练习几乎没有什么效果。
...
要实现这些特性，主要是要编写范围严格的问题。当提示符的范围太广时，您通常会遇到问题：提取通常会缺少重点目标；您可能会产生不准确或不一致的答案；您可能会发现提示符难以处理。但是，写范围狭窄的问题出人意料地困难。你需要把知识分解成离散的部分，这样你就可以把这些部分重新构建起来，作为提取练习的提示。这种分解也使复习更有效率。课程表将快速删除常规练习中容易的材料，同时确保您经常检查您认为最难的组件。
...
也就是说，「与鹦鹉押韵」可能是一个有用的记忆工具，可以添加到**答案**中，提供一个联想，以帮助你在未来：
...
好吧，你可以尝试这样的方法。「一批鸡汤里有多少鸡骨？」 但什么是「一批」？这个问题还不够精确。我们可以求助于特定的配方，问 「Andy 的鸡汤配方里有多少鸡骨？」 但这实际上不是我们想知道的：菜谱指出，你要根据你使用的鸡骨数量调整菜谱。写出好的提示往往涉及解释，这是创造超越明确书写的理解的第一步。
...
现在我们已经有了关于成分的提示，让我们来看看菜谱的步骤。这是**程序性**知识——执行具体任务所需的知识，更多的是知道如何做而不是知道做什么。
...
这样做更好。但是将程序编辑下来有助于我们对这些知识进行一些观察。首先：几个关键词（或词组）承载了该程序的关键细节。**缓慢地**到**小火**，然后保持**小火 90 分钟**。如果你知道这三个粗体字，你基本上就知道这个过程了。其他的话只是一个骨架。这给我们带来了第二个看法：第一和第四步不值得写提示。如果你知道什么是高汤（我们会讲到这一点），你就知道你是在锅里熬制一堆材料。当汤汁炖完后，最后一步就是常识了。你还会做什么？
...
这将产生大约 1.5 夸脱鸡汤在冰箱里可以保存一个星期，或在冰箱里无限期地保存。冷藏后，鸡汤上会有一层脂肪，在使用鸡汤前请将其撇去，并在任何美味的烹饪情况下用脂肪代替油或黄油。
