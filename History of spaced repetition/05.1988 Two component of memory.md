...
[长期记忆的双组分模型](https://supermemo.guru/wiki/two component_model_of_long-term_memory)是 [SuperMemo](https://supermemo.guru/wiki/SuperMemo) 的基础，并在 [SM-17 算法](https://supermemo.guru/wiki/Algorithm_SM-17)中明确表示。它区别于[稳定性](https://supermemo.guru/wiki/Stability)和[可检索性](https://supermemo.guru/wiki/Retrievability)。这仍然是学习理论中一个鲜为人知的典型事实，即一个人可以流利，但记忆力仍然很差。
...
上世纪80年代中期，我对[间隔重复](https://supermemo.guru/wiki/Spaced_repetition)的研究让我很快意识到，一个变量*强度*不足以描述存储在长期记忆中的知识的状态。我们需要两个变量来区分事实从内存中检索的难易程度。
...
我的结论是，存储在长期记忆中的知识的状态可以用两个变量来描述：[稳定性](https://supermemo.guru/wiki/Stability)和[可检索性](https://supermemo.guru/wiki/Retrievability)。记忆的稳定性告诉您内存在存储中可以持续多长时间。可检索性告诉你回忆一段知识是多么容易。在记忆研究中，这两个变量常常被混为一谈。
...
令人惊讶的是，在我观察了三十年之后，记忆的两个组成部分的概念仍然渗透到主流的记忆研究中。2018年，大多数论文忽略了这两个变量的分离，仍然依赖于单一的“记忆强度”概念。这导致了巨大的困惑和研究进展缓慢。
...
1988年1月9日，我在计算机模拟课上的一篇论文中首次描述了[长期记忆的两个组成部分](https://supermemo.guru/wiki/Two_component_model_of_memory)。在同一篇论文中，我用了一种不相关的推理方法，得出结论:[陈述性](https://supermemo.guru/wiki/ative_learning)和[过程性学习](https://supermemo.guru/wiki/programural_learning)必须涉及不同的电路。
...
如果您暂停一分钟，那么这两个组件的整个思想应该是非常明显的。如果您在检查之后立即使用两个[memory](https://supermemo.guru/wiki/Item)，一个使用短[optimization interval](https://supermemo.guru/wiki/Optimum_interval)，另一个使用长优化interval，那么这两个内存状态必须不同。它们都可以被完美地回收(maximum [retrievability](https://supermemo.guru/wiki/Retrievability))，而且它们还需要在内存中保存的时间上有所不同(different [stability](https://supermemo.guru/wiki/Stability))。我很惊讶我找不到任何关于这个主题的文献。然而,如果文献没有提到的存在(最优区间)(记忆),这个看似显而易见的结论可能会躲在另一个看似显而易见的想法:的进展增加对最优区间间隔的评论。这是一个可爱的例子，说明人类的进步是如何渐进和令人痛苦的缓慢。众所周知，我们不善于打破常规。烛台下是最黑暗的地方。这一弱点可以通过网络通讯的爆炸来打破。我提倡减少[同行评审](https://supermemo.guru/wiki/Peer_review)，并进行更大胆的假设。我提到了一个来自罗宾·克拉克关于阿尔茨海默氏症的论文中的绝妙例子。严格的同行审查让人想起[普鲁士的学校教育](https://supermemo.guru/wiki/Prussian_education_system):在追求完美的过程中，我们失去了创造力，失去了人性，最终失去了生活的乐趣。
...
1988年2月19日，当我第一次向我的老师卡图斯基博士提出我的想法时，他并没有给我留下太深刻的印象，但他给了我一张计算机模拟学分的通行证。顺便说一句，不久之后，Katulski成为[SuperMemo 1.0 for DOS](https://supermemo.guru/wiki/SuperMemo_1.0_for_DOS)的首批用户之一。
...
在我的[Master's Thesis](https://supermemo.guru/wiki/Master's _thesis)(1990)中，我添加了一个稍微正式的证明，证明这两个组件的存在(参见[next](https://supermemo.guru/wiki/History_of_spaced_repetition_(print)# proof))。我论文的那一部分没有引起注意。

In 1994, J. Kowalski [wrote in Enter](http://www.super-memory.com/articles/kowal.htm), Poland:

1994年，J.科瓦尔斯基[wrote In Enter](http://www.supermemory.com/articles/kowal.htm)，波兰:
...
>
> 我们已经到了这样一个阶段，对记忆的进化解释表明，它使用的原理是增加间隔和[间隔效应](https://supermemo.guru/wiki/Spacing_effect)。除了进化论的推测外，还有什么证据可以证明这种记忆模型吗?在他的博士论文中，沃兹尼亚克广泛地讨论了记忆的分子方面，并提出了一个假设模型，描述了学习过程中突触发生的变化。本文提出的新元素是内存跟踪的[稳定性](https://supermemo.guru/wiki/Stability)和[可检索性](https://supermemo.guru/wiki/Retrievability)之间的区别。这不能用来支持[SuperMemo](https://supermemo.guru/wiki/SuperMemo)的有效性，因为一个简单的事实是，正是SuperMemo本身奠定了假设的基础。然而，越来越多的分子证据似乎与稳定性-可检索性模型相吻合，同时，该模型为导致SuperMemo的假设的正确性提供了支持。简单地说，可检索性是记忆的一种属性，它决定了突触对刺激反应的效率水平，从而引发学习行为。可检索性越低，您就越不可能回忆起对问题的正确回答。另一方面，稳定性反映了早期重复的历史，并决定了记忆痕迹能够维持的时间范围。记忆的稳定性越高，恢复能力下降到零的时间也就越长，也就是记忆永久丢失的时间。根据沃兹尼亚克的说法，当我们第一次学习某样东西的时候，我们会在编码特定刺激反应关联的突触中体验到稳定性和可检索性的轻微提高。随着时间的推移，可收回性迅速下降;相当于遗忘的现象。同时，记忆的稳定性保持在大致相同的水平。然而，如果我们在可检索性下降到零之前重复这种关联，可检索性将恢复其初始值，而稳定性将增加到一个新的水平，显著高于初级学习时的水平。在下一次重复发生之前，由于稳定性的提高，可检索性以较慢的速度下降，在遗忘发生之前，重复之间的间隔可能要长得多。记忆的另外两个重要特性也应该注意到:(1)当可检索性高时，重复没有能力增加稳定性(间隔效应)，(2)遗忘时，稳定性迅速下降
...
我们与[Janusz Murakowski博士](https://supermemo.guru/wiki/Janusz_Murakowski)和[Edward Gorzelanczyk](https://supermemo.guru/wiki/Edward_Gorzelanczyk)在[1995](http://supermemory.com/english/2vm.htm)发表了我们的想法。村上春树完善了数学证明。Gorzelanczyk充实了分子模型。我们没有从科学界听到太多的热情和反馈。记忆的两种成分就像葡萄酒，越老，味道越好。我们一直想知道它什么时候会得到更广泛的认可。毕竟，我们并不是生活在[孟德尔](https://en.wikipedia.org/wiki/Gregor_Mendel)的时代，把一块好宝石藏在某个晦涩的档案中。[间隔重复]的[百万用户](https://supermemo.guru/wiki/Exponential_adoption_of_spaced_repetition)[间隔重复](https://supermemo.guru/wiki/Spaced_repetition)即使只有0.1%的人对这个理论感兴趣，他们也会听说我们的两个组件。今天，甚至[SuperMemo](https://supermemo.guru/wiki/Algorithm_SM-17)中的[最新算法](https://supermemo.guru/wiki/SuperMemo)都是基于双组件模型的，而且运行起来很有魅力。具有讽刺意味的是，用户往往会涌向[更简单的解决方案](https://supermemo.guru/wiki/Algorithm_SM-2)，那里隐藏着人类记忆的所有机制。即使在[supermemo.com](http://supermemo.com/)上，我们也确保不会用屏幕上过多的数字吓到客户。这样，我们就可以在渴望简单的用户中更好地采用模型的指导功能。
...
记忆的两个组成部分的概念在以前的研究中有相似之处，特别是在[Bjork](https://supermemo.guru/wiki/Bjork)。
...
在20世纪40年代，科学家们研究了习惯强度和反应强度作为行为的独立组成部分。这些概念后来在比约克的废用理论中得到了重新表述。赫伯特•西蒙(Herbert Simon)似乎在1966年的论文中注意到了(需要内存稳定性变量)(https://supermemo.guru/wiki/Herbert_Simon_predicted_two_component_model_of_memory)。1969年，[Robert Bjork](https://supermemo.guru/wiki/Robert_Bjork)提出了“强度悖论”:回忆的概率与回顾的记忆效果之间的反向关系。注意，他在[two component model](https://supermemo.guru/wiki/Spacing_effect)中重述了[间距效应](https://supermemo.guru/wiki/Two_component_model)，这离明确内存变量之间的区别只有一步之遥。这导致(比约克的废弃的新理论》(1992))(https://www.researchgate.net/profile/Robert_Bjork/publication/281322665_A_new_theory_of_disuse_and_an_old_theory_of_stimulus_fluctuation/links/58b6f20945851591c5d55e96/A-new-theory-of-disuse-and-an-old-theory-of-stimulus-fluctuation.pdf),区分存储能力和检索能力。它们与[可检索性](https://supermemo.guru/wiki/Retrievability)和[稳定性](https://supermemo.guru/wiki/Stability)非常相似，但对两者区别背后的机制的解释略有不同。最引人注目的是，Bjork认为当[可检索性](https://supermemo.guru/wiki/Retrievability)降到0时，稳定的内存仍然保留(在我们的模型中，稳定性变得不确定)。在细胞水平,比约克可能是正确的,至少有一段时间,但练习(SuperMemo) (https://supermemo.guru/wiki/SuperMemo)显示(完全忘记)(https://supermemo.guru/wiki/Complete_forgetting)的力量,同时,从神经的角度来看,留住记忆在停止使用会非常低效的独立的(稳定)(https://supermemo.guru/wiki/Stability)。最后但并非最不重要的是，Bjork根据连通性定义了存储强度，这与我认为好学生的情况非常接近:[coherence](https://supermemo.guru/wiki/Coherence)影响[稳定性](https://supermemo.guru/wiki/Stability)。
...
为什么记忆的两个组成部分还没有进入主流研究领域?我断言,如果人类思维往往是短视的,我们都是,通过设计,科学的思想可以通过艰苦的工作,真正绞窄的发布或灭亡,争夺赠款、层次结构、利益冲突,同行评审(https://supermemo.guru/wiki/Peer_review),教学义务,甚至行为准则。记忆研究者倾向于生活在“记忆强度”的单一维度中。在这个维度上，他们无法真正理解需要研究才能解决问题的分子和神经过程的真正动力学。具有讽刺意味的是，进步可能来自那些倾向于从事人工智能或神经网络工作的人。[Demis Hassabis](https://en.wikipedia.org/wiki/Demis_Hassabis)或[Andreas Knoblauch](https://scholar.google.de/citations?生物学家将需要倾听数学和/或计算机科学的语言。
...
[长期内存的两个组件模型](https://supermemo.guru/wiki/Two_component_model_of_long-term_memory)是 [Algorithm SM-17](https://supermemo.guru/wiki/Algorithm_SM-17) 的基础。算法SM-17 的成功，是该模型正确性的最终实践证明。
...
记忆的两个组成部分价值的实际变化图提供了演变中的记忆状况的概念可视化：
...
>
> **图片：**记忆状态随着时间的推移而发生变化。横轴表示跨越整个重复历史的时间。顶部面板显示可检索性(第十次幂，R10，便于分析)。灰色网格的可检索性指标分别为R=99%、R=98%等。中间的面板显示海军的最佳间隔。重复日期用蓝色竖线标记，并以水标记。当R越过90%线时，最佳区间的末端用红色竖线表示(仅当区间大于最佳区间时)。下面板显示稳定性(以ln(S)/ln(days)表示，便于分析)。从图中可以看出，当稳定性较低时，早期重复后的可检索性下降较快(指数级)，但在第7次复习后的10年时间里，可检索性仅从100%下降到94%。所有的值都来自一个实际的重复历史和内存的三个组件模型。
