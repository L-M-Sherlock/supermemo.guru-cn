...
[长期内存的双组分模型](https://supermemo.guru/wiki/two component_model_of_long-term_memory)是[SuperMemo](https://supermemo.guru/wiki/SuperMemo)的基础，并在[Algorithm SM-17](https://supermemo.guru/wiki/Algorithm_SM-17)中明确表示。它区别于[稳定性](https://supermemo.guru/wiki/Stability)和[容易检索](https://supermemo.guru/wiki/Retrievability)。这仍然是一个鲜为人知的和典型的学习理论的事实，一个人可以流利，但仍然记得很差。
...
我的结论是，存储在长期记忆中的知识的状态可以用两个变量来描述:[stability](https://supermemo.guru/wiki/Stability)和[retrievability](https://supermemo.guru/wiki/Retrievability)。内存的稳定性告诉您内存在存储中可以持续多长时间。可检索性告诉你回忆一段知识是多么容易。在记忆研究中，这两个变量常常被混为一谈。
...
[长期内存的两个组件模型](https://supermemo.guru/wiki/Two_component_model_of_long-term_memory)是[Algorithm SM-17](https://supermemo.guru/wiki/Algorithm_SM-17)的基础。算法SM-17的成功，是该模型正确性的最终实践证明。
...
有一个重要的结论直接来自于SuperMemo理论，那就是有两个，而不是像人们通常认为的那样，一个是描述突触电导率和一般记忆的自变量。为了说明这种情况，让我们再次考虑突触记忆的calpain模型。从模型中可以明显看出，作者假设只有一个自变量是描述突触电导率所必需的。钙的流入、钙蛋白酶的活性、fodrin的降解和谷氨酸受体的数量都是这种变量的例子。注意，上述所有参数都是相关的，即知道其中一个参数就可以计算出其他所有参数;显然，只有在我们能够构造出相关公式的情况下。参数的依赖性是所有参数之间因果关系的直接结果。
...
这里有一份由[Murakowski]提交的证据(https://supermemo.guru/wiki/Janusz_Murakowski):
...
[SuperMemo](https://supermemo.guru/wiki/SuperMemo)一直基于[two component model of memory](https://supermemo.guru/wiki/Two_component_model_of_memory)，后者在过去三十年中以一种越来越显式的形式出现。Eqn中的常数C2。(2)在Murakowski的证明中，上述代表[stability increase](https://supermemo.guru/wiki/Stability_increase)。在2018年，稳定性的提高体现在[SuperMemo](https://supermemo.guru/wiki/SuperMemo)中，矩阵*SInc[]*。C2说，为了满足可接受遗忘水平的标准，学习中应该增加多少重复间隔。实际上，C2不是常数。这取决于许多因素。其中最重要的是:
